...
private void Query1_CondExpr(CondExpr[] expr) {
  ...
  private void Query2_CondExpr(CondExpr[] expr, CondExpr[] expr2) {
    ...
    expr[0].type1 = new AttrType(AttrType.attrSymbol);
    expr[0].type2 = new AttrType(AttrType.attrSymbol);
    expr[0].operand1.symbol = new FldSpec (new RelSpec(RelSpec.outer),1); // S
    expr[0].operand2.symbol = new FldSpec (new RelSpec(RelSpec.innerRel),1); // R
    ...
    expr2[0].operand1.symbol = new FldSpec (new RelSpec(RelSpec.outer),2); // R
    expr2[0].operand2.symbol = new FldSpec (new RelSpec(RelSpec.innerRel),1); // B
    ...
    expr2[1].operand1.symbol = new FldSpec (new RelSpec(RelSpec.innerRel),3);// B
...
//    expr[1].operand1.symbol = new FldSpec (new RelSpec(RelSpec.innerRel),4);
    ...
    expr[1].operand2.real = (float)40.0;
    

    expr[1].next = new CondExpr();
    expr[1].next.op   = new AttrOperator(AttrOperator.aopLT);
    expr[1].next.next = null;
    expr[1].next.type1 = new AttrType(AttrType.attrSymbol); // rating
    expr[1].next.operand1.symbol = new FldSpec ( new RelSpec(RelSpec.outer),3);
    expr[1].next.type2 = new AttrType(AttrType.attrInteger);
    expr[1].next.operand2.integer = 7;
  ...
  private void Query7_CondExpr(CondExpr[] expr) {
	    ...
	    expr[1].type2 = new AttrType(AttrType.attrReal);
	    expr[1].operand1.symbol = new FldSpec (new RelSpec(RelSpec.innerRel), 4);
	    expr[1].operand2.real = (float)34.2;
	 
	    expr[2] = null;
	  }
  

  //  two predicates inequality joins
 private void Query8_CondExpr(CondExpr[] expr) {
  ...
  public void Query1() {
    ...
    FldSpec [] proj_list = new FldSpec[2]; // what input fields go where in the output tuple
  ...
  private void Query7() throws JoinsException, IndexException, InvalidTupleSizeException, InvalidTypeException, PageNotReadException, TupleUtilsException, PredEvalException, SortException, LowMemException, UnknowAttrType, UnknownKeyTypeException, IOException, Exception 
  {
	    ...
	    // Sailors, Boats, Reserves Queries.
	    System.out.print 
	      ("Test/extend the existing NLJ with single predicate inequality joins\n"
	       + "Query: \n"
	       + "  SELECT   s1.sname\n"
...
//	    System.out.println("btf in doc:" + btf);
	    ...
	    try {
	      nlj = new NestedLoopsJoins (
	    		  	  Stypes, 4, Ssizes,  //  Array containing field types of R (S1)
	    		  	 Stypes, 4, Ssizes, //Array containing field types of S (S2)
					  10,
					  am, "sailors.in", // access hfapfile for right i/p to join
					  outFilter, null, proj1, 1);  // proj1 -->  shows what input fields go where in the output tuple
	    }
	    catch (Exception e) {
	      System.err.println ("*** Error preparing for nested_loop_join");
	      System.err.println (""+e);
...
//	    System.out.println ("======================================"); 
  ...
  public void Query8() {
	    ...
	    // Sailors, Boats, Reserves Queries.
	    System.out.print 
	      ("Test/extend NLJ with two predicates inequality join \n"
	       +"Query: \n"
	       + "  SELECT   S1.sname\n"
	       + "  FROM     Sailors S1, Sailors S2\n"
	    ...
	    short [] JJsize = new short[1];
	    JJsize[0] = 30;
	    FldSpec []  proj1 = {   //shows what input fields go where in the output tuple
	       new FldSpec(new RelSpec(RelSpec.outer), 2),
	    };
...
//	    System.out.print ("After Building btree index on sailors.sid.\n\n");
	    try {
	      am = new IndexScan ( b_index, "sailors.in",
				   "BTreeIndex", Stypes, Ssizes, 4, 4,
				   Sprojection, null, 1, false);
	      System.out.println("The value in am is :" + am);
	    }
	    ...
	    NestedLoopsJoins nlj = null;
	    try {
	      nlj = new NestedLoopsJoins (Stypes, 4, Ssizes, // Array containing field types of R.
					  Stypes, 4, Ssizes, // Array containing field types of S
					  10,
					  am, "sailors.in",  // relationName  access heapfile for right i/p to join
					  outFilter, null, proj1, 1);  // proj1 shows what input fields go where in the output tuple
	    }
	    catch (Exception e) {
	      System.err.println ("*** Error preparing for nested_loop_join");
	    ...
	    QueryCheck qcheck8 = new QueryCheck(8);
