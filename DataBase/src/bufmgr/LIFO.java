...
class LIFO extends  Replacer {
   ...
   */
    ...
    private int numBuffers = mgr.getNumBuffers();
...
//    update(frameNo);
 ...
 public int pick_victim() throws BufferPoolExceededException
 {
    ...
    * as victim and no need to remove current page out of buffer pool
    */
    ...
    /**
     * if the buffer is full (have no free frame), the last not pinned item of @frame 
     * will be selected as the victim
     */
    for ( int i = numBuffers - 1; i > -1; i-- ) {
         frame = frames[i];
